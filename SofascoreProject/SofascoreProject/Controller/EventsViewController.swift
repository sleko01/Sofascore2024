...
class EventsViewController: UIViewController, BaseViewProtocol {
    ...
    private func refreshData() {
        if let team = team {
            apiClient.getAllTeamEvents(teamId: team.id.description, onlyOne: onlyOne ?? false) { tournaments, error in
                if self.onlyOne ?? false {
                    let tournament = [tournaments.first!]
                    self.tournaments = tournament
                    self.tournaments[0].matchList = [self.tournaments[0].matchList.first!]
                } else {
